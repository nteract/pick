...
class Kernel(SingletonConfigurable):
    ...
    def schedule_dispatch(self, priority, dispatch, *args):
        ...
        self.msg_queue.put_nowait((priority, idx, dispatch, args,))
    ...
    def start(self):
        ...
        if self.control_stream:
            self.control_stream.on_recv(
                partial(
                    self.schedule_dispatch, CONTROL_PRIORITY, self.dispatch_control,
                ),
                copy=False,
            )
        ...
        for s in self.shell_streams:
            ...
            s.on_recv(
                partial(
                    self.schedule_dispatch, SHELL_PRIORITY, self.dispatch_shell, s,
                ),
                copy=False,
            )
    ...
    def execute_request(self, stream, ident, parent):
        ...
        reply_content = yield gen.maybe_future(
            self.do_execute(code, silent, store_history, user_expressions, allow_stdin,)
        )
    ...
    def complete_request(self, stream, ident, parent):
        ...
        completion_msg = self.session.send(
            stream, "complete_reply", matches, parent, ident
        )
    ...
    def shutdown_request(self, stream, ident, parent):
        ...
        self._shutdown_message = self.session.msg(u"shutdown_reply", content, parent)
    ...
    def is_complete_request(self, stream, ident, parent):
        ...
        reply_content = json_clean(reply_content)
        reply_msg = self.session.send(
            stream, "is_complete_reply", reply_content, parent, ident
        )
        self.log.debug("%s", reply_msg)
    ...
    def _abort_queues(self):
        ...
        self.schedule_dispatch(
            ABORT_PRIORITY, self._dispatch_abort,
        )
    ...
    def _send_abort_reply(self, stream, msg, idents):
        ...
        md.update(status)
        self.session.send(
            stream, reply_type, metadata=md, content=status, parent=msg, ident=idents,
        )
    ...
    def getpass(self, prompt="", stream=None):
        ...
        return self._input_request(
            prompt, self._parent_ident, self._parent_header, password=True,
        )
    ...
    def raw_input(self, prompt=""):
        ...
        return self._input_request(
            str(prompt), self._parent_ident, self._parent_header, password=False,
        )
    ...
    def _input_request(self, prompt, ident, parent, password=False):
        ...
        self.session.send(
            self.stdin_socket, u"input_request", content, parent, ident=ident
        )
